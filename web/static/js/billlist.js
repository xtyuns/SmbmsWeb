...
function deleteBill(obj){
	$.ajax({
		type:"GET",
		url:path+"/admin/bill.do?method=delete&billid="+obj.attr("billid"),
		dataType:"json",
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/admin/bill.do?method=view&billid="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/admin/bill.do?method=modify&billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
