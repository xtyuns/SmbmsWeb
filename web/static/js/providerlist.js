...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/admin/provider.do?method=delete&proid="+obj.attr("proid"),
		dataType:"json",
		success:function(data){
			...
			}else if(data.delResult == "hasBills"){
				//alert("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.delResult+"】条订单，不能删除");
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】存在订单，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/admin/provider.do?method=view&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/admin/provider.do?method=modify&proid="+ obj.attr("proid");
	});
