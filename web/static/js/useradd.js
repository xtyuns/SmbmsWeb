...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/admin/role.do?method=getRoleList",//请求的url
		...
		error:function(data){//当访问时候，404，500 等非200的错误状态码
			validateTip(userRole.next(),{"color":"red"},imgNo+" 获取用户角色列表失败",false);
		}
	});
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"POST",//请求类型
			url:path+"/ajax.do",//请求的url
			data: {method:"ucexist", userCode:userCode.val()},
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.userCode === "notExist"){//账号未存在, 可以使用
			...
			error:function(data){//当访问时候，404，500 等非200的错误状态码
				validateTip(userCode.next(),{"color":"red"},imgNo+" 账号检查请求失败",false);
			}
		});
